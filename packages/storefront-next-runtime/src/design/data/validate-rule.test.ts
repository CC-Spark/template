...
describe('validateRule', () => {
    ...
    describe('campaign qualifiers', () => {
        test('passes when shopper has the required campaign qualifier', () => {
            ...
            expect(validateRule(rule, context)).toBe(true);
        });
        ...
        test('fails when shopper lacks the campaign', () => {
            ...
            expect(validateRule(rule, makeContext())).toBe(false);
        });
        ...
        test('fails when shopper has campaign but wrong promotion', () => {
            ...
            expect(validateRule(rule, context)).toBe(false);
        });
        ...
        test('all campaign qualifiers must match (AND logic)', () => {
            ...
            expect(validateRule(rule, partialContext)).toBe(false);
            ...
            expect(validateRule(rule, fullContext)).toBe(true);
        });
        ...
        test('fails when no context is provided', () => {
            ...
            expect(validateRule(rule, null)).toBe(false);
            expect(validateRule(rule, undefined)).toBe(false);
        });
    });

    describe('isActiveForLocale', () => {
        test('fails when isActiveForLocale is false', () => {
            const rule: VisibilityRuleDef = { isActiveForLocale: false };
            expect(validateRule(rule)).toBe(false);
        });

        test('fails when isActiveForLocale is false even with passing schedule and customer groups', () => {
            vi.useFakeTimers();
            vi.setSystemTime(new Date('2026-06-15T00:00:00.000Z'));

            const rule: VisibilityRuleDef = {
                isActiveForLocale: false,
                customerGroups: ['vip'],
                schedule: {
        ...
        test('passes when isActiveForLocale is true and no other conditions', () => {
            const rule: VisibilityRuleDef = { isActiveForLocale: true };
    ...
    describe('customer groups', () => {
        test('passes when shopper belongs to the required group', () => {
            const rule: VisibilityRuleDef = { isActiveForLocale: true, customerGroups: ['vip'] };
            const context = makeContext({ customerGroups: { vip: true } });
            expect(validateRule(rule, context)).toBe(true);
        });
        ...
        test('all customer groups must match (AND logic)', () => {
            ...
            expect(validateRule(rule, partial)).toBe(false);

            const full = makeContext({ customerGroups: { vip: true, 'early-access': true } });
            expect(validateRule(rule, full)).toBe(true);
        });
    ...
    describe('schedule', () => {
        ...
        test('does not require context for schedule-only rules', () => {
            ...
            expect(validateRule(rule, null)).toBe(true);
        });
    ...
    describe('campaign-based rules ignore schedule, customer groups, and isActiveForLocale', () => {
    ...
    describe('non-campaign rules check isActiveForLocale, schedule, AND customer groups', () => {
    ...
    describe('empty / absent rule fields', () => {
        test('passes when rule has no conditions besides isActiveForLocale', () => {
            expect(validateRule({ isActiveForLocale: true })).toBe(true);
        });
        ...
        test('passes when rule has empty arrays', () => {
            ...
            expect(validateRule(rule, makeContext())).toBe(true);
        });
    });
});
